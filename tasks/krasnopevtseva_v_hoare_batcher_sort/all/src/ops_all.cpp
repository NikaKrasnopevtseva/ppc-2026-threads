...
void KrasnopevtsevaVHoareBatcherSortALL::BatcherMergeBlocksStep(int *left_pointer, int &left_size,
...
#pragma omp parallel for default(none) shared(pointers, sizes, pack, step, thread_input_size, par_if_greater) if ((thread_input_size / step) > par_if_greater)
...
bool KrasnopevtsevaVHoareBatcherSortALL::RunImpl() {
  ...
  if (rank == 0) {
    ...
    MPI_Scatterv(temp.data(), send_counts.data(), displs.data(), MPI_INT,
                 local_data.data(), my_n, MPI_INT, 0, MPI_COMM_WORLD);
  } else {
  ...
  MPI_Gatherv(local_data.data(), my_n, MPI_INT,
              global_data.data(), send_counts.data(), displs.data(), MPI_INT,
              0, MPI_COMM_WORLD);
